...
- `kernel` robust fits now store analytical `mu_vcov` and `delta_vcov`, so
  scalar summaries and exact-grid contrasts can use covariance-aware endpoint
  differences without requiring bootstrap;
...
- Optional multiplier/bootstrap replicate curves are now stored for
  `dml_flex` multiplier fits and kernel bootstrap fits; kernel robust and
  bootstrap curve covariance matrices are stored, and `dml_flex` BLP-spline
  fits store level and derivative grid covariance matrices.
...
- Avoid reporting endpoint-difference SEs as formal contrast inference unless a
  stored curve covariance matrix or replicate-curve contrast is available. This
  is now satisfied for kernel robust/bootstrap contrasts and for
  `dml_flex + signal_map = "blp_spline"`; it remains a caution for non-BLP
  `dml_flex` maps when only pointwise fallback is available.
