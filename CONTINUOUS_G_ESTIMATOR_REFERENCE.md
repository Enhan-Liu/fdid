...
Timestamp: 2026-05-30, America/Chicago
...
- analytical kernel curve inference is unit-level robust; when
  `vartype = "bootstrap"` and a cluster column is available, kernel bootstrap
  resampling is cluster-aware.
...
- computed by a local sandwich estimator at each `g0`;
- `se_mu` for level;
...
```text
est$event$Estimate = mean(theta_hat(eval_g))
est$event$Std.Error = mean(se_theta(eval_g))
```

This is a compatibility summary. The primary substantive output is the curve in
`curve_event`.
...
- kernel robust pointwise normal intervals are not displayed or labeled as
  uniform bands;
...
scalar $est$event is grid-average theta.
```
...
2. Level-comparison and fixed-G derivative reporting are first-class through
   ...
   replicate-based and curve-covariance-based grid contrasts; `dml_flex` with
   `signal_map = "blp_spline"` supports curve-covariance-based contrasts.
   Theorem-level efficient contrast inference remains open for other curve
   paths.
...
8. Kernel bootstrap resampling is cluster-aware when a cluster column is
   available. Analytical kernel curve sandwiches and `dml_flex` curve
   inference are not currently cluster-adjusted.
