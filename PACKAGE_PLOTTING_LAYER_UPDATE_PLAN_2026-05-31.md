...
2026-05-31 revision note: this version supersedes
`PACKAGE_PLOTTING_LAYER_UPDATE_PLAN_2026-05-30.md` for sequencing. It first
moved the formal fixed-reference continuous-G contrast coefficient plot ahead
of the package-wide plot-data / ggplot/autoplot modernization. That narrower
feature has now been implemented, so the broader modernization remains the next
larger plotting decision.
...
Sequencing decision: the DML-kernel base-graphics alignment pass remains the
foundation, but it is no longer the next plotting code pass because it has
already been implemented. The fixed-reference contrast coefficient plot has
also been implemented. The package-wide ggplot/autoplot plan can now be
revisited as the next larger plotting decision.
...
Implementation note, 2026-05-31: the manual tutorial example for
`theta(g) - theta(g_ref)` was promoted to a first-class R plotting feature
before the larger ggplot/autoplot work. This is a base-graphics,
inference-aware addition to the current continuous-G plotting surface, not a
dependency change.

Implementation note, later on 2026-05-31: the fixed-reference contrast
coefficient plot has been implemented as `plot.fdid(type = "contrast")` for
kernel and `dml_flex`. The next plotting decision is therefore the package-wide
plot-data / `fdid_autoplot()` modernization described below.

Implementation note, final contrast refinement on 2026-05-31: the contrast
plot now includes the reference row by default, supports character `ref.g`
...
Original workflow: Docs Only.

Implementation workflow, later on 2026-05-31: Code Change. The
fixed-reference contrast slice from this plan has now been executed with a
planner pass, builder pass, tester pass, scriber/documentation pass, and
review pass. The broader package-wide ggplot/autoplot modernization remains
unimplemented.
...
4. DML adaptation pass: split DML plotting into scalar, overlap, curve, and
   diagnostic surfaces.
5. 2026-05-31 revision pass: add a first-class fixed-reference contrast
   coefficient plot as the immediate pre-ggplot code slice.
6. Reviewer pass: review this plan for completeness before implementation.

The original planning pass intentionally did not edit R code, tests, tutorials,
`NEWS.md`, or `HANDOFF.md`. The later implementation slice edited R code,
tests, help, estimator reference, project log, TODO, this plan, and the latest
tutorial visualization chapter. It still did not edit `NEWS.md` or
`HANDOFF.md`.
...
- keeps the current base-graphics `plot.fdid()` and `plot.fdid_list()` behavior
  backward compatible;
- adds a first-class fixed-reference continuous-G contrast coefficient plot
  before the broader package-wide modernization;
...
New planned plotting target:

- fixed-reference continuous-G contrast coefficients:
  `theta(g_j) - theta(g_ref)` for `dml_flex` and
  `mu(g_j) - mu(g_ref)` for `kernel`, evaluated at a small set of selected
  `G` values and displayed as coefficients with 95 percent intervals.
...
### Implemented `plot.fdid(type = "contrast")`

Source objects: `x$eval_g`, `x$curve_event`, and the reporting helper
`fdid_contrast()`.

Implemented behavior:

- use base graphics in the first pass;
- available for `method = "kernel"` and `method = "dml_flex"`;
- draw coefficient-style point estimates and 95 percent intervals for
  contrasts against one reference level;
- respect `ci = FALSE` by suppressing contrast CI bars;
- default to level-curve contrasts, not derivative contrasts;
- include the reference value as an algebraic zero row by default;
- choose the fitted-grid median reference for level targets when `ref.g` is
  not supplied, and the first selected quantile target for quantile targets;
- support reference rules such as `ref.g = "median"`, `"mean"`, `"min"`,
  `"max"`, and `"first"`;
- choose a small default set of target `G` values from the stored grid, with
  displayed rows capped by `max.contrast`;
- support quantile targets through `target.type = "quantile"`, including
  `target.g = 5`, `target.g = "5"`, or `target.g = "q5"` for five
  quantile-selected rows;
- allow users to override target points through `contrast.values` or a new
  clearer alias such as `target.g`;
- use `fdid_contrast()` for each displayed contrast so inference follows the
  existing rule order: stored covariance, stored replicate curves, stored
  bands, then clearly labeled pointwise fallback;
- label the reference value and the inference rule used for each coefficient;
- return `invisible(NULL)` through the base `plot.fdid()` path.

Important interpretation:

- this is not a new estimator;
- it is a visualization of selected relative level effects from stored
  continuous-G curve estimates;
- intervals are only as strong as the inference source used by
  `fdid_contrast()`;
- a full smooth contrast curve with simultaneous bands remains a later
  extension.
...
- raw, dynamic, overlap, curve, and list plots do not share a common plot-data
  contract;
- continuous-G curve and fixed-reference contrast plots now have reusable
  internal data helpers, but the rest of the plotting layer does not;
...
- fixed-reference continuous-G contrasts now have a first-class base
  coefficient plot, but not yet a returned ggplot/autoplot object;
...
- continuous-G contrast coefficient plots: `target_g`, `reference_g`,
  `estimate`, `std.error`, `conf.low`, `conf.high`, `inference`, `note`,
  `method`, `curve`, `target_estimand`, `source_rule`, `is_reference`;
...
added `diff.values` and `contrast.values` for continuous-G curves. The
2026-05-31 contrast-plot slice should connect this idea to actual
`fdid_contrast()` output: marked values can become selected target levels in a
coefficient plot relative to a fixed reference level. The later ggplot path
should preserve the same data contract.
...
### Immediate Base-Graphics Addition

Add a first-class continuous-G contrast plot to `plot.fdid()`:
...
- `type = "contrast"` is valid only for `method = "kernel"` and
  `method = "dml_flex"`;
- `ref.g = NULL` selects the fitted `eval_g` value closest to the center of the
  grid, unless a better package-wide convention is chosen during
  implementation;
- user-supplied `ref.g` can be off-grid when `interpolate = "linear"`;
- `contrast.values = NULL` and `target.g = NULL` select up to `n.contrast`
  displayed rows from the stored `eval_g` grid, including the default
  reference row when `include.ref = TRUE`;
- `n.contrast` defaults to 7 and should not exceed `max.contrast = 10` in the
  automatic selection path, with both counts interpreted as displayed rows;
- `target.type = "quantile"` selects displayed rows from support or `eval_g`
  quantiles, with the first selected quantile used as the default reference
  when `ref.g = NULL`;
- user-supplied `contrast.values` should be honored, but the plot should warn
  or message when more than 10 points are requested because the display becomes
  hard to read;
- `inference = "auto"` delegates each target comparison to `fdid_contrast()`;
- the plotted intervals are 95 percent intervals by default, using the fitted
  object's `alpha` when available;
- coefficients are shown as points with vertical 95 percent CIs and a zero
  reference line.

Example target display:

```r
plot(fit, type = "contrast", ref.g = median(fit$eval_g))
plot(fit, type = "contrast", ref.g = 0.5, contrast.values = c(0.2, 0.8))
```

This immediate addition should be implemented before `fdid_autoplot()` or any
package-wide ggplot work. It answers the applied need for relative level-effect
plots while staying inside the existing base plotting contract.

### Phase-1 Public Function For Package-Wide Modernization
...
fdid_autoplot(
  object,
  type = c("raw", "dynamic", "scalar", "overlap", "curve", "contrast",
           "diagnostics"),
  ...
)
```
...
- `type = "contrast"` is valid for continuous-G curve methods and returns the
  same coefficient table as the base contrast plot, rendered as a ggplot object
  once the ggplot path exists;
...
- build the contrast plot data by calling `fdid_contrast()` point-by-point
  rather than duplicating contrast SE logic in the plotting code;
...
.fdid_ggplot_contrast(contrast_data, style, labels, ...)
...
### Continuous-G Reference-Level Contrast Plot

Implemented goal:

- formalize the tutorial's fixed-reference contrast display as an R package
  plotting feature.
...
- `method = "kernel"`;
- `method = "dml_flex"`;
- level-contrast estimands; target rows may be chosen by numeric levels or by
  quantile selection.
...
mu(g_j) - mu(g_ref)          for kernel
```

Data builder:

- validate that the object has `eval_g` and `curve_event`;
- choose `reference_g`:
  - if `ref.g` is supplied, use it;
  - if `ref.g` is a supported character rule, resolve it on the fitted grid;
  - otherwise use the stored grid point closest to the grid median for level
    mode or the first selected quantile target for quantile mode;
- choose displayed target points:
  - if `contrast.values` is supplied, use those values;
  - if `target.type = "quantile"`, resolve quantile targets from observed
    support within the fitted range or from `eval_g`;
  - otherwise choose up to `n.contrast` grid-spaced values from `eval_g`,
    including the reference row when `include.ref = TRUE`;
  - default `n.contrast = 7L`;
  - automatic selection should not exceed `max.contrast = 10L`;
- call `fdid_contrast(x, g0 = reference_g, g1 = target_g, ...)` for each
  non-reference target value;
- construct the reference row directly as an algebraic zero;
- return one row per target with `target_g`, `reference_g`, `estimate`,
  `std.error`, `conf.low`, `conf.high`, `inference`, `note`, and
  `is_reference`.

Base renderer:

- coefficient-style plot, not a smooth curve;
- x-axis can be target `G` values, or an ordered categorical axis labeled by
  target `G`; choose the version that reads better under irregular grids;
- y-axis is the relative level effect;
- point estimates through `points()` or `plot(..., pch = 19)`;
- 95 percent intervals through vertical segments/arrows;
- zero reference line;
- optional vertical or textual marker for `reference_g`;
- captions or legends should identify the inference source when mixed sources
  occur across target values.

ggplot renderer:

- `geom_pointrange()` or `geom_errorbar()` plus `geom_point()`;
- `geom_hline(yintercept = 0)`;
- optional annotation for `reference_g`;
- same data builder as the base path.

Inference rules:

- do not compute SEs inside the renderer;
- delegate to `fdid_contrast()`;
- covariance-aware contrasts use stored curve covariance when available;
- replicate-based contrasts use stored bootstrap or multiplier replicate
  curves when available;
- fallback pointwise endpoint intervals must remain labeled approximate;
- if no usable interval is available, plot the point estimate and warn clearly.

Tests:

- default target selection has no more than 10 rows;
- user-supplied `contrast.values` are respected;
- `ref.g` is included as a zero reference row by default;
- `include.ref = FALSE` restores the old no-reference-row display;
- character `ref.g` rules work and unsupported rules error clearly;
- quantile target mode works for counts, probability vectors, and shorthand
  such as `target.g = "5"` or `target.g = "q5"`;
- `kernel` bootstrap contrast plot uses covariance or replicate inference when
  available;
- `dml_flex` with `signal_map = "blp_spline"` uses covariance-aware contrast
  inference when available;
- unsupported methods error clearly;
- base plot restores graphics parameters;
- ggplot contrast path returns a ggplot object when implemented.
...
3. Add `.fdid_plot_data_contrast()` for fixed-reference continuous-G contrast
   coefficient plots.
4. Add raw, dynamic, overlap, support, and fdid-list data builders.
5. Add structural tests for every data builder.

Acceptance:

- every plot type can be represented as canonical data;
- default contrast plot data has no more than 10 target rows;
- contrast plot data is built from `fdid_contrast()` output rather than
  duplicating inference calculations;
- data helpers are independent of graphics devices;
- no public plotting behavior changes.

### Phase 1A: Add Base Fixed-Reference Contrast Plot

Actions:

1. Add `type = "contrast"` to `plot.fdid()`.
2. Add public arguments needed by the contrast plot: `ref.g`,
   `n.contrast`, and a target-point argument. Prefer reusing
   `contrast.values` if this does not make the existing vertical-marker
   semantics ambiguous; otherwise add a clearer alias such as `target.g`.
3. In the renderer, draw point estimates plus 95 percent CIs for
   `theta(g_j) - theta(g_ref)` or `mu(g_j) - mu(g_ref)`.
4. Use `fdid_contrast()` for estimates, SEs, CIs, inference labels, and notes.
5. Add focused tests and roxygen/Rd updates.
6. Update the estimator reference, project log, TODO, and the latest tutorial
   visualization chapter after implementation.

Acceptance:

- `plot(fit, type = "contrast")` works for kernel and `dml_flex`;
- the default display has no more than 10 target points;
- explicit `ref.g` and explicit target points work;
- intervals use covariance/replicate/band/pointwise fallback labels from
  `fdid_contrast()`;
- unsupported methods error clearly;
- base plotting remains dependency-free and backward compatible.
...
4. Keep the current curve and contrast base paths aligned with shared
   continuous-G plot data.
...
5. Add `type = "contrast"` for continuous-G fixed-reference coefficient plots
   if the base contrast path is already stable.
6. Add object-class and layer-data tests.
...
- `fdid_autoplot(fit, type = "scalar")` returns a `ggplot` object for scalar
  DML methods;
- `fdid_autoplot(fit, type = "contrast")` returns a `ggplot` object for
  kernel and `dml_flex`;
...
7. Ensure curve plots and contrast coefficient plots share target-value
   semantics where `contrast.values` is used.
...
- contrast plot data reference selection, target selection, inference labels,
  and no-more-than-10 default row count;
...
- contrast coefficient plots render as pointrange-style layers with a zero
  reference line;
...
- `plot(fit, type = "contrast")` for kernel and `dml_flex`;
...
- `dml_incremental` scalar plots identify the observed-population average
  derivative target.
- contrast coefficient plots inherit inference labels from `fdid_contrast()`;
- covariance-aware contrast plots identify covariance or BLP-spline covariance
  inference rather than endpoint-independent pointwise inference.
...
- fixed-reference contrast coefficient plot semantics;
...
- state that ggplot output uses the same stored estimates and interval fields
  as base plots;
- state that `type = "contrast"` is a selected-point coefficient plot for
  contrasts against `ref.g`, not a newly estimated smooth curve;
...
- add fixed-reference contrast plotting as the immediate plotting-layer code
  slice before package-wide ggplot work;
...
- interval semantics in Notes should change only if displayed uncertainty
  semantics change;
- if the contrast plot becomes a recommended reporting display, add a short
  package-facing note that it visualizes selected
  `theta(g_j) - theta(g_ref)` or `mu(g_j) - mu(g_ref)` contrasts already
  covered by the reporting layer;
...
- add a short example of `plot(fit, type = "contrast", ref.g = ...)` in the
  visualization chapter after the R implementation exists;
...
- for contrast plots, display or store the `fdid_contrast()` inference label
  used for each coefficient;
...
### Contrast-Selection Risk

Risk:

- too many automatically selected target values can make the coefficient plot
  unreadable, and a poor default reference value can encourage misleading
  comparisons.

Mitigation:

- default to a small number of displayed target points;
- keep the automatic default at or below 10 points;
- choose a central reference level by default;
- let users supply both `ref.g` and target values explicitly;
- document that estimates are relative to the chosen reference level.
...
Resolved contrast-plot decisions from the 2026-05-31 refinement:

- `target.g` is the clearer contrast target alias; `contrast.values` remains
  supported for level contrast targets and remains the marker argument in curve
  plots.
- explicit user-supplied target lists longer than `max.contrast` are allowed
  with a warning; automatic and quantile selection use `max.contrast` as the
  displayed-row cap.
- level-mode `ref.g = NULL` uses the fitted `eval_g` median rule; quantile mode
  with `ref.g = NULL` uses the first selected quantile target.

## Recommended Next Implementation Slice

The DML-specific Phase A from `DML_PLOT_UPDATE_PLAN_2026-05-30.md` has already
been implemented. The fixed-reference contrast coefficient plot has also now
been implemented. This completed slice added:

1. Add a small internal contrast plot-data helper that calls `fdid_contrast()`.
2. Add `plot.fdid(type = "contrast")` for `kernel` and `dml_flex`.
3. Add `ref.g`, target-point selection, `n.contrast`, and `max.contrast`
   controls.
4. Make the default target selection display no more than 10 points.
5. Draw coefficient estimates with 95 percent CIs and a zero reference line.
6. Add tests for target selection, inference labels, covariance/replicate
   paths, unsupported methods, and graphics-state restoration.
7. Update roxygen/Rd, `CONTINUOUS_G_ESTIMATOR_REFERENCE.md`,
   `CONTINUOUS_G_PROJECT_LOG.md`, `TODO.md`, and the latest tutorial
   visualization chapter.

The next plotting code pass, if the package-wide plan is activated, should be
the package-wide plot-data / ggplot modernization. Its first code pass should
be deliberately small:

1. Add internal plot-data helpers for raw, dynamic, fdid-list, scalar DML, and
   curve.
2. Refactor base raw/dynamic/fdid-list plots to use the helpers.
3. Add `ggplot2` to `Suggests`.
4. Add exported `fdid_autoplot()` for raw, dynamic, scalar DML, fdid-list, and
   the already implemented contrast coefficient data.
5. Add structural tests and ggplot object tests.
6. Update roxygen/Rd, `CONTINUOUS_G_PROJECT_LOG.md`, `TODO.md`, and the
   estimator reference.

Defer to later code passes:
...
Because current user interest is now concentrated on reference-level contrast
visualization, the package-wide ggplot/autoplot plan should remain an explicit
later decision rather than becoming an automatic next step.
...
- fixed-reference continuous-G contrast coefficient plots are implemented for
  kernel and `dml_flex` before the package-wide ggplot/autoplot layer;
...
- users can obtain ggplot objects for raw, dynamic, overlap, curve, and
  fdid-list plots;
- users can obtain ggplot objects for fixed-reference continuous-G contrast
  coefficient plots;
